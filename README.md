...
- [EarthCARE (MAAP)](https://portal.maap.eo.esa.int):
  only products from the EarthCAREL1Validated_MAAP, EarthCAREXMETL1DProducts10_MAAP, EarthCAREL2Validated_MAAP,
  JAXAL2Validated_MAAP, and EarthCAREOrbitData_MAAP collections.
...
To download data via MAAP you will need to register for an account at
...
You will then need to login and [retrieve a 90-day offline token](https://portal.maap.eo.esa.int/ini/services/auth/token/90dToken.php).

You will have to define an environment variable `MAAP_OFFLINE_TOKEN` that
contains the value of this offline token.

You can set this environment variable within a linux shell using:

```
export MAAP_OFFLINE_TOKEN="xxxxxxxxxxxxx"
```
(replacing the xxx values with your token)

or from within python, using:

```
os.environ["MAAP_OFFLINE_TOKEN"] = "xxxxxxxxxxxxx"
```
